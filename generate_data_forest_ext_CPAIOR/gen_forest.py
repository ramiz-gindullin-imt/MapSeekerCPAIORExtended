...
This file is to generate all sequences of forests of size between 1 and n vertices with n growing to 2 up to maxn vertices
"""
...
def write_ext_table(combinaison,ind_comb,dic_comb_dic_value,lowup):
    ...
    if lowup == "up":
        df.to_csv("ext_forest/up"+gen_name_datatable(combinaison)+".csv")
    else:
        df.to_csv("ext_forest/low"+gen_name_datatable(combinaison)+".csv")
    return df
...
def gen_forest_csv_tables(combinaisons_list,list_carac_names,lowup):
    ...
    with open("foret_avec_point_isole_"+str(taille_max_foret)+".csv", newline='') as csvfile:
