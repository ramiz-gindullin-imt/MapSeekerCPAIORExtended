...
:- use_module(gen_bool_formula_test_1_core_model).
:- use_module(gen_cond_formula).
:- use_module(gen_options_for_formulae_test_1_core_model).
:- use_module(utility).
...
formula_generator1(bool, KindCombi, CallMap, _OptionsBoolCond, TableEntries, Options, _ColSetsBool, ShiftBool, Table, Col, CurMaxCost,
    ...
    formula_generator_bool_conds(BoolParams, _ColSetsBool,              % from the options describing a Boolean formula,
                                 ShiftBool,                             % generates the corresponding Boolean formula
                                 t(_NbUnary,_NbBinary,_NbTernary,_AdjustMaxOccAttrToOne),
                                 FormulaPattern1),
    ...
    %    )
    %),
    gen_occ_vars(2, LenA1, MaxOccOfAnAttr, OccVars, _OccVarsAlone),      % generate occ. vars for gcc constraint
