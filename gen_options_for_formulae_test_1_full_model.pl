...
gen_options_lists(KindCombi, col(Table,OutputCol), Kind, OptionsBoolCond, PrevInputParams, InputParams, ForceToUseOnlySmallFd,
                  TableEntries, Mode, [bool(ListsOptions)], SetsBool) :-
    (KindCombi \== model_seeker ->
        gen_options_lists_by_type(bool,    KindCombi, col(Table,OutputCol), Kind, Mode, OptionsBoolCond, PrevInputParams, InputParams, ForceToUseOnlySmallFd, ListsOptions),
...
gen_options_lists(KindCombi, col(Table,OutputCol), Kind, OptionsBoolCond, PrevInputParams, InputParams, ForceToUseOnlySmallFd,
    ...
    (KindCombi \== model_seeker ->
        gen_options_lists_by_type(cond_ex, KindCombi, col(Table,OutputCol), Kind, Mode, OptionsBoolCond, PrevInputParams, InputParams, ForceToUseOnlySmallFd, [ListsOptionsCond, _ListsOptionsCondEx]),
        ...
        ((ListsOptionsCond = [], ListsOptions3 = []) ->
            SetsBool = []
        ;
            gen_value_sets_wrt_boolean_operators(KindCombi, TableEntries, Mode, InputParams, col(Table,OutputCol), [], [], SetsBool)
        ),
...
gen_options_lists_by_type(CondBool, KindCombi, col(Table,_OutputCol), _Kind, Mode, OptionsBoolCond, _PrevInputParams, InputParams, ForceToUseOnlySmallFd, ListsOptions) :-
    ...
    ;
        (CondBool = bool ->                            % if generate options for Boolean formulae
            ...
            call(user:table_info(Table,_,_,Negated,Oplus,NbTerms)),
            memberchk([bool([neg(_), op(_), nb_terms(_,_), conds(Conds), use_mods(_)])], OptionsBool),
            OptionsFormula = [[bool([neg(Negated), op([Oplus]), nb_terms(NbTerms,NbTerms), conds(Conds), use_mods(2)])]]
            /*OptionsBoolCond = OptionsBool-_,           % get set of potential Boolean formulae that was generated once and for all
            (Mode = preprocessed(Min,Max, _, _) ->
                 Range is Max-Min
            ;
