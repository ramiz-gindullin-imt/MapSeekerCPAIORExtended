...
:- use_module(gen_bool_formula_test_1_full_model).
:- use_module(gen_formula_test_1_full_model).
:- use_module(gen_formula_term).
:- use_module(gen_formula_write).
:- use_module(gen_candidate_tables).
:- use_module(gen_options_for_formulae_test_1_full_model).
...
top(0) :-
    ...
    open('performance_test_stats_full_model.pl', write, FOut),
    assert(fout(FOut)),
	format(FOut, ':- multifile performance/7.~n:- dynamic performance/7.~n~n', []),
...
search_secondaries([col(Table,Col)|R], OptionsBoolCond, MaxN, CallMap, Sout) :-
   ...
   tab_get_arity(col(Table,_), NAll), NInputs is NAll - 1,
   tab_get_nb_rows(col(Table,_), NbRows),
   call(table_info(Table,KindCombi,Map,_Negated,Oplus,NbTerms)),
   call(fout(FOut)),
   format(FOut,'performance(~w,~w,~w,~w,~w,~w,~w).~n',[Table, NInputs-NbRows, KindCombi, Map, Oplus, NbTerms, Runtime]),
